...
Public Class frmAdminStudInfo
    ...
    Public Sub search()
        ...
        Try
            conDB.Close()
            conDB.Open()
            conCommand = New MySqlCommand("SELECT * FROM tblstudinfo WHERE sid=@sid OR sname=@sid", conDB)
    ...
    Private Sub cmbDep_DropDown(sender As Object, e As EventArgs) Handles cmbDep.DropDown
        Dim dbtable As New DataTable
        Try
            conDB.Close()
            conDB.Open()
        ...
        While conReader.Read
            cmbDep.Items.Add(conReader("depart"))
        End While
        Catch ex As Exception
        MsgBox(ex.Message)
        Finally
    ...
    Private Sub cmbLA_SelectedIndexChanged(sender As Object, e As EventArgs) Handles cmbLA.SelectedIndexChanged
        txtCL.Text = cmbLA.Text
    End Sub

    Private Sub txtTel_KeyPress(sender As Object, e As KeyPressEventArgs) Handles txtTel.KeyPress
        If Char.IsDigit(e.KeyChar) Or e.KeyChar = Convert.ToChar(Keys.Back) Or e.KeyChar = Convert.ToChar(Keys.Tab) Then
            e.Handled = False
        Else
            e.Handled = True
        End If
    ...
    Private Sub btnUpdate_Click(sender As Object, e As EventArgs) Handles btnUpdate.Click
        Try
            ...
            conCommand = New MySqlCommand("UPDATE tblstudinfo Set sid=@sid,fname=fname,mname=@mname,sname=@sname,gend=@gend,
    ...
    Sub fillgrid()
        Try
            conDB.Close()
            conDB.Open()
            conCommand = New MySqlCommand("Select sid,fname,mname,sname,gend,cl FROM tblstudinfo WHERE sname <> 'n/a'", conDB)
            conReader = conCommand.ExecuteReader(CommandBehavior.CloseConnection)
            dgv1.Rows.Clear()
            While conReader.Read
    ...
    Private Sub btnSave_Click(sender As Object, e As EventArgs) Handles btnSave.Click
        ...
        Try
            For Each ctl As Control In grpInfo.Controls
                If ctl.Text.Length = 0 And ctl.Tag <> "Middlename" And ctl.Tag <> "cd" And ctl.Tag <> "cl" And TypeOf ctl IsNot Button And TypeOf ctl IsNot PictureBox Then
            ...
            Else
                conDB.Close()
                conDB.Open()
                conCommand = New MySqlCommand("SELECT sid,sname FROM tblstudinfo WHERE sid =@sid AND sname = @sname ", conDB)
                conCommand.Parameters.AddWithValue("@sid", txtID.Text)
                ...
                Else
                    ...
                    conCommand = New MySqlCommand("INSERT INTO tblstudinfo(sid,fname,mname,sname,gend,dob,tel,dad,dam,day,la,cl,cd,course,depart)
